...
function toServerFormatter(value, row, index) {
    var server_ips = row.toServer;
    str = '  <select class="form-control btn-warning GroupSelector">\n'
    str += '<option class="btn-success" value="' + server_ips[0] + '" selected>' + server_ips[0] + '</option>';
    for (var i = 1; i < server_ips.length; i++) {
        ip = server_ips[i];
        str += '<option class="btn-success" value="' + server_ips[i] + '">' + server_ips[i] + ' </option>';
    }
    str += '</select>';
...
function migrateFormatter(value, row, index) {
    // var group = row.group;
    // var isStart = row.task_start;
    // var isFinished = row.task_Finish;
    // var ProgressValue = row.progress_value;
    ...
    return [
        '<a  role="button"  class="label btn  btn-info  MigrateBTN">Migrate</a>',
    ].join('');
}
...
var migrate_operateEvent = {

    'click .MigrateBTN': function (e, value, row, index) {
        var ModalID = row.ModalID;
        if (confirm('Sure to migrate this?') === true) {
            $('#WaitModal').modal({backdrop: 'static'})
            $('#WaitModal').modal('show')

            $.ajax({
                url: "/DBCPOnline/MigrateModal/" + ModalID,
                headers: {
