                ...
                if re.findall(pattern, file):
                    ...
                    if re.findall(pattern_e1_json, file):
                        with open(file, 'r') as load_f:
                            ...
                            if dic['EchoTime']:
                                ...
                                exit(1)
                        command = 'mv ' + file + ' ' + root_path + '/fmriprep/input/sub-' + str(subjectid) + '/fmap/sub-' + str(subjectid) + '_magnitude1.json'
                        os.system(command)
                    pattern_e1_nii = re.compile(r'_e1.nii$')
                    if re.findall(pattern_e1_nii, file):
                        command = 'mv ' + file + ' ' + root_path + '/fmriprep/input/sub-' + str(subjectid) + '/fmap/sub-' + str(subjectid) + '_magnitude1.nii'
                        os.system(command)
                    ...
                    if re.findall(pattern_e2_json, file):
                        with open(file, 'r') as load_f:
                            ...
                            if dic['EchoTime']:
                                ...
                                logging.error('No EchoTime filed found in ' + file)
                                exit(1)
                        command = 'mv ' + file + ' ' + root_path + '/fmriprep/input/sub-' + str(subjectid) + '/fmap/sub-' + str(subjectid) + '_magnitude2.json'
                        os.system(command)
                    pattern_e2_nii = re.compile(r'_e2.nii$')
                    if re.findall(pattern_e2_nii, file):
                        command = 'mv ' + file + ' ' + root_path + '/fmriprep/input/sub-' + str(subjectid) + '/fmap/sub-' + str(subjectid) + '_magnitude2.nii'
                        os.system(command)
                    ...
                    if re.findall(pattern_ph_json, file):
                        ...
                        with open(file, 'w') as load_f:
                            ...
                            load_f.close()
                        command = 'mv ' + file + ' ' + root_path + '/fmriprep/input/sub-' + str(subjectid) + '/fmap/sub-' + str(subjectid) + '_phasediff.json'
                        os.system(command)
                    pattern_ph_nii = re.compile(r'_ph.nii$')
                    if re.findall(pattern_ph_nii, file):
                        command = 'mv ' + file + ' ' + root_path + '/fmriprep/input/sub-' + str(subjectid) + '/fmap/sub-' + str(subjectid) + '_phasediff.nii'
                        os.system(command)
        ...
        for folder in os.listdir(parentFolder):
            ...
            for file in os.listdir(os.getcwd()):
                if re.findall(pattern, file):
                    ...
                    pattern_ph_nii = re.compile(r'.nii$')
                    if re.findall(pattern_ph_nii, file):
                        fmriName = root_path + '/fmriprep/input/sub-' + str(subjectid) + '/func/sub-' + str(subjectid) + '_task-rest_bold.nii'
...
def DoFreeSurfer():
    ...
    if len(os.listdir(root_path + '/freesurfer/input/sub-' + subjectid)) == 1:
        command = 'source $FREESURFER_HOME/SetUpFreeSurfer.sh && recon-all -s ' + root_path + '/freesurfer/output/' + 'sub-' + str(subjectid) + ' -i ' + root_path + '/freesurfer/input/sub-' + subjectid + '/sub-' + str(subjectid) + '_T1w.nii -all'
    else:
        index = 1
    ...
    command = 'cp -r /usr/local/freesurfer/subjects/sub-' + subjectid + ' ' + root_path + '/freesurfer/output/sub-' + str(subjectid)
    os.system(command)
    return
